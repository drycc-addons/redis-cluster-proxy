...
curl -s --data-binary @bin/valkey-cluster-proxy -H "Content-Type: application/octet-stream" -H "${GITHUB_AUTH_HEAD}" -X POST "${GITHUB_ASSET_UPLOAD_URL}?name=valkey-cluster-proxy-${CI_SYSTEM_PLATFORM}"